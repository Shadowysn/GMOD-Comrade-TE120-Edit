...
local main_mat = "effects/ar2_altfire2"
local blur_mat = "effects/ar2_altfire2b"
local flicker_mat = "effects/combinemuzzle2c"
local halo_mat1 = "effects/combinemuzzle1b_nocull"
local halo_mat2 = "effects/combinemuzzle2b_nocull"
local halo_mat2alt = "effects/combinemuzzle2c"

function ENT:DrawMotionBlur()
	local color = Vector(0,0,0)

	local vecDir = self.Entity:GetPos() - self.vecLastOrigin
	local speed = Vector:Normalize( vecDir )

	speed = math.Clamp( speed, 0, 32 )

	local stepSize = math.min( ( speed * 0.5 ), 4.0 )
...
function ENT:DrawFlicker()
	...
	render.DrawSprite(self.Entity:GetPos(), self.flRadius * rand2, self.flRadius * rand2, color)
end
...
function ENT:DrawSprites()
	...
	--[[if m_bHeld then
		...
		local roll = self.SpawnTime
		
		--DrawHaloOrientedG(self.Entity:GetPos(), self.flRadius + sinOffs, color, roll)
		render.SetMaterial(main_mat)
		render.DrawSprite(self.Entity:GetPos(), self.flRadius + sinOffs, self.flRadius + sinOffs, color)
	--end

	self.vecLastOrigin = GetAbsOrigin()
end
