...
physexplode:SetKeyValue("radius", 3350)
...
self:Remove()
	...
	local inradius = ents.FindInSphere(data.HitPos, 450)
	
	for key, ent in ipairs(inradius) do
		if ent:GetClass() != "sent_combine_ball_medium1" then
			if ent:IsValid() then
				local x = (ent:GetPos().x - data.HitPos.x) * (1050 / data.HitPos:Distance(ent:GetPos()))
				local y = (ent:GetPos().y - data.HitPos.y) * (1050 / data.HitPos:Distance(ent:GetPos()))
				local z = (ent:GetPos().z - data.HitPos.z + 0) * (1050 / data.HitPos:Distance(ent:GetPos()))
...
timer.Simple(0.01,function()
	util.BlastDamage(self.Entity, self:OwnerCheck(), self.Entity:GetPos(), 50, 1500)
	end)
